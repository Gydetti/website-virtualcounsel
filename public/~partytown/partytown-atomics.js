...
((e) => {
	...
	const s = (e) => (11 === e.nodeType && e.host ? "#s" : e.nodeName);
	const a = () =>
	...
	const l = { Graphics: "g", SVG: "svg" };
	const u = { preserveBehavior: !1 };
	...
	let k = 0;
	const A = e.parent;
	const M = document.implementation.createHTMLDocument();
	const x = A.partytown || {};
	const P = `${x.lib || "/~partytown/"}`;
	const H = (e, t, r) => {
		const n = ((e, t) => {
			return (
				(r = e),
	...
	const V = (e) => new ("detail" in e ? CustomEvent : Event)(e.type, e);
	const z = (e, t, r, n) => {
												...
												o = o[s[t]] =
													t + 1 < r(s)
														...
														: (() => {
																let e = null;
																if (i) {
																	const { methodOrProperty: t, thisObject: r } =
																		((e, t) => {
																			let r = e;
	...
	const K = (e, t, r) => {
		if (!y.has(r)) {
			...
			const u = i.pushState.bind(i);
			const d = i.replaceState.bind(i);
			const p = (r, i, o, s) => () => {
				e.postMessage([
					v.LocationUpdate,
	...
	const te = (e, r) => (void 0 !== e[r] ? new e[r](t) : 0);
	let re;
	(async (e) => {
		...
		return (n, i) => {
			...
			if (o === v.MainDataRequestFromWorker) {
				const e = (() => {
					...
					const r = M.createComment("");
					const n = M.createDocumentFragment();
					const i = M.createElement("p").attachShadow({ mode: "open" });
					const o = te(A, "IntersectionObserver");
					const s = te(A, "MutationObserver");
					const a = te(A, "ResizeObserver");
					const c = A.performance;
					const l = A.screen;
					const u = [
						[A.history],
						[c],
