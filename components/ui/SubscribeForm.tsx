...
export function SubscribeForm() {
  ...
  return (
    <form onSubmit={handleSubscribe} className="w-full md:w-auto flex flex-col sm:flex-row gap-3">
      ...
      />
      <Button type="submit" disabled={status === "loading" || status === "success"} className="bg-white text-primary hover:bg-gray-200">
        {status === "loading" ? "…sending" : status === "success" ? "✅ Subscribed" : "Subscribe"}
      </Button>
      {status === "error" && <p className="mt-2 text-red-400 text-sm">Oops—something went wrong.</p>}
    </form>
  )
} 