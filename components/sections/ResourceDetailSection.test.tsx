...
describe("ResourceDetailSection", () => {
  ...
  it("renders text block content from resource sections", () => {
    ...
    const textBlockContent = resource.sections.find(
      (s) => s.type === "text",
    )?.content;
    if (textBlockContent) {
      ...
      throw new Error(
        "Sample resource data does not contain expected text block for testing.",
      );
    }
  });
  ...
  it("renders form block with title from resource sections", () => {
    ...
    if (formBlockData?.title) {
      ...
      console.warn(
        "Sample resource form block might not have a title for testing.",
      );
      expect(
