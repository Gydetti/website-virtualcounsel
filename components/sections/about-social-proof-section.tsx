...
export default function AboutSocialProofSection({ badgeText, heading, socialProof }: AboutSocialProofSectionProps) {
  return (
    <Section id="about-social-proof" className="py-12 bg-gray-50">
      <LazySection animation="slide-up" delay={0} className="max-w-4xl mx-auto text-center">
        {badgeText && <Badge className="mb-4 bg-primary text-primary-foreground">{badgeText}</Badge>}
        {heading && <h3 className="section-title mb-6">{heading}</h3>}
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-8">
          {socialProof.map((item) => (
            <div key={item.id} className="p-6 bg-white rounded-lg shadow hover:shadow-md transition-shadow">
              {item.image?.src && (
