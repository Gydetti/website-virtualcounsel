...
async function main() {
	...
	try {
		...
		if (err.code === "ENOENT") {
			console.warn(
				`⚠️ Raw images directory not found at ${rawBase}, skipping image optimization.`,
			);
	...
	for (const category of categories) {
		...
		for (const file of files) {
			...
			try {
				...
				console.warn(
					`⚠️ Could not generate blur for ${inputPath}: ${err.message}`,
				);
			}
			...
			if (["jpg", "jpeg", "png"].includes(ext)) {
				...
				try {
					...
					const webpBuffer = await sharp(webpOutputPath)
						.resize({ width: 20 })
						.toBuffer();
					...
					console.warn(
						`⚠️ Could not convert to WebP for ${inputPath}: ${err.message}`,
					);
				}
			}
		}
