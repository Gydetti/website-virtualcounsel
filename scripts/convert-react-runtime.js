...
module.exports = function transformer(fileInfo, api) {
    ...
    const used = new Set();
    const memberPaths = root.find(j.MemberExpression, {
        object: { name: 'React' },
    ...
    if (used.size > 0) {
        const importDecls = root.find(j.ImportDeclaration, { source: { value: 'react' } });
        const specifiersToAdd = Array.from(used).sort().map(name => j.importSpecifier(j.identifier(name)));
        if (importDecls.size() > 0) {
            const importPaths = importDecls.paths();
            for (const path of importPaths) {
                const existing = path.node.specifiers.filter(s => s.type === 'ImportSpecifier').map(s => s.imported.name);
