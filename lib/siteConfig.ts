...
try {
  ...
  if (error instanceof z.ZodError) {
    console.error(
      "Error validating site.config.local.ts:",
    ...
    throw new Error(
      "Site config validation failed (ZodError). Check console for details.",
    );
  }
  console.error(
    "An unexpected error occurred during site config validation:",
    error,
  );
  throw new Error(
    "Site config validation failed (UnknownError). Check console for details.",
  );
}
